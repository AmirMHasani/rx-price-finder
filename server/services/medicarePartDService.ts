// Server-side Medicare Part D API service (bypasses CORS)
 ...
 * Server-side only to bypass CORS restrictions
 */
export async function searchPartDByName(
  ...
  try {
    ...
    if (!apiKey) {
      console.error('[Part D] CMS API key not configured');
      return null;
    }
    ...
    console.log('[Part D] Searching for:', cleanName);
    ...
    if (!response.ok) {
      console.log('[Part D] Brand name search failed, trying generic name');
      ...
      if (!response.ok) {
        console.error('[Part D] Generic name search also failed:', response.status);
        return null;
      }
    }
    ...
    if (data.results && data.results.length > 0) {
      console.log('[Part D] Found:', data.results[0].gnrc_name || data.results[0].brnd_name);
      return data.results[0];
    }

    console.log('[Part D] No results found');
    return null;
  } catch (error) {
    console.error('[Part D] Error fetching data:', error);
    return null;
  }
}
